...
public class RatingController {
    ...
    public ModelAndView getUserRatingPage(@PathVariable String username, Model model) {
        List<RatingDto> ratings = ratingService.getRatingsByUser(username);

        ModelAndView modelAndView = new ModelAndView("UserRating"); // UserRating.html 페이지 지정
        modelAndView.addObject("ratings", ratings); // 평점과 후기 데이터를 모델에 추가
        modelAndView.addObject("username", username); // 사용자 이름을 모델에 추가

        return modelAndView; // ModelAndView를 반환하여 UserRating.html 페이지로 데이터를 전달하고 렌더링
    }
